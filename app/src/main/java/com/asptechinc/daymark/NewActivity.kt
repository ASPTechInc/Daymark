...
import com.asptechinc.daymark.ui.TagSelectionAdapter
...
class NewActivity : AppCompatActivity() {
    ...
    private var availableTags = mutableListOf<Tag>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        saveButton.setOnClickListener {
            ...
            intent.putExtra("tagIds", ArrayList(selectedTagIds))
    ...
    private fun populateSpinners(
        ...
        this.availableTags = tags.toMutableList()
        this.selectedTagIds = selectedTagIds.toMutableList()
        ...
        // Selected tags
        for (tag in tags) {
            if (this.selectedTagIds.contains(tag.id)) {
                val chip =
                    Chip(this).apply {
                        text = tag.name
                        isCheckable = false
                        ...
                        setOnCloseIconClickListener {
                            this@NewActivity.selectedTagIds.remove(tag.id)
                            populateSpinners(
                                categoryNames,
                                categoryIds,
                                tags,
                                selectedCategoryId,
                                this@NewActivity.selectedTagIds,
                            )
                        }
                    }
        ...
        val addChip =
            Chip(this).apply {
                text = getString(R.string.tag_selection_add_tag)
                chipIcon = ContextCompat.getDrawable(this@NewActivity, R.drawable.ic_plus)
                setOnClickListener {
                    showTagSelectionDialogue()
                }
            }
        tagChipGroup.addView(addChip)
    }

    private fun showTagSelectionDialogue() {
        val dialogueView = LayoutInflater.from(this).inflate(R.layout.dialogue_tag_selection, null)
        val searchInput = dialogueView.findViewById<EditText>(R.id.tag_search_input)
        val recyclerView = dialogueView.findViewById<RecyclerView>(R.id.tag_list_recycler_view)

        val adapter = TagSelectionAdapter(availableTags, selectedTagIds.toSet())
        ...
        val dialogue =
            MaterialAlertDialogBuilder(this)
                .setTitle(R.string.tag_selection_title)
                .setView(dialogueView)
                .setPositiveButton(R.string.dialogue_action_apply) { _, _ ->
                    selectedTagIds = adapter.getSelectedIds().toMutableList()
                    populateSpinners(
